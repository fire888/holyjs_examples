...
export const createDoorData = (root, lineData, l, mode = 'simple') => {
    if (leftProfile === null) {
        leftProfile = [...pos]
        M.rotateVerticesY(leftProfile, -Math.PI / 4)
        M.translateVertices(leftProfile, 12, 0, 5)
        rightProfile = [...pos]
        M.rotateVerticesY(rightProfile, Math.PI / 4)
        M.translateVertices(rightProfile, -12, 0, 5)
    }
    const v = []
    const c = []
    const b = []
    ...
    const copyV = [...v]
    M.inverseVertexOrder(copyV)
    M.scaleVertices(copyV, -1, 1, 1)
    M.translateVertices(copyV, l, 0, 0)
    v.push(...copyV)
    u.push(...u)
    const copyC = [...c]
    c.push(...copyC)
    ...
    const mirrorV = [...v]
    M.inverseVertexOrder(mirrorV)
    M.scaleVertices(mirrorV, 1, 1, -1)
    v.push(...mirrorV)
    c.push(...c)
    u.push(...u)
